...
export class ScreenUtils {
    ...
    public static calculateScreenMetrics(defaultWidth: number, defaultHeight: number,
        orientation: ScreenOrientation = ScreenOrientation.LANDSCAPE, maxGameWidth?: number, maxGameHeight?: number): ScreenMetrics {
        // Just to give some explanation as to the numbers and colors in the included background;
        // The GREEN is the safe area and will be displayed fully on any device and is based on 16:10 aspect ratio,
        // build your actual gameplay here
        ...
        // The RED is the extra area that will be visible on devices with a 16:9 aspect ratio (iPhone 5 and above)
        // (this is probably the most common ratio overall...)
        // The GREY area will most likely never be seen, unless some device has a really odd aspect ratio
        // (and with Android, I wouldn't be surprised if there is a few out there)
        ...
        // Calculate the max width and max height if not provided; ratios are based off iPad (4:3)
        // and iPhone 5+ (16:9) as the extremes in both width and height
        if (!maxGameWidth || !maxGameHeight) {
            if (orientation === ScreenOrientation.LANDSCAPE) {
                this.screenMetrics.maxGameWidth = Math.round(this.screenMetrics.defaultGameWidth *
                    (MAX_GAME_WIDTH / DEFAULT_GAME_WIDTH));
                this.screenMetrics.maxGameHeight = Math.round(this.screenMetrics.defaultGameHeight *
                    (MAX_GAME_HEIGHT / DEFAULT_GAME_HEIGHT));
            } else {
                this.screenMetrics.maxGameWidth = Math.round(this.screenMetrics.defaultGameWidth *
                    (MAX_GAME_HEIGHT / DEFAULT_GAME_HEIGHT));
                this.screenMetrics.maxGameHeight = Math.round(this.screenMetrics.defaultGameHeight *
        ...
        const defaultAspectRatio: number = ((orientation === ScreenOrientation.LANDSCAPE) ?
            (DEFAULT_GAME_WIDTH / DEFAULT_GAME_HEIGHT) : (DEFAULT_GAME_HEIGHT / DEFAULT_GAME_WIDTH));
...
export class StringUtils {
    public static toCamelCase(str: string): string {
        return str.replace(/[^A-Za-z0-9]/g, ' ').replace(/^\w|[A-Z]|\b\w|\s+/g, function (match: string, index: number): string {
    ...
    /**
     * Replace n of ${i} ... ${n} occurrences in a string.
     * The string can only be interpolated once, as it'll start from 1 and goes up to i.
     * @param stringToChange The string where things need to get inserted into.
     * @param replaceThings The things to insert into the string.
     * @returns The interpolated string.
     */
    public static interpolateString(stringToChange: string, ...replaceThings: any[] ): string {
        for (let i: number = 1; i <= replaceThings.length; i++) {
            stringToChange = stringToChange.replace('${' + i + '}' , String(replaceThings[i - 1]));
        }
