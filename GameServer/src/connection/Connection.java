...
public class Connection
{
    ...
    /**
     * Send a message to the client that the game has finished. So the player goes from the game screen to the end screen.
     */
    ...
    /**
     * Send a message to the client that the game session has ended. So the player goes from the end screen to the menu screen.
     */
    public void sendEnd()
    {
        socket.send("end");
    ...
    /**
     * Send a message to the client with all the valid precondition tokens that the teacher solution uses.
     * @param validPreTokens The String[] with all the lexed tokens from the teacher solution.
     */
    ...
    /**
     * Send a message to the client with all the valid postcondition tokens that the teacher solution uses.
     * @param validPostTokens The String[] with all the lexed tokens from the teacher solution.
     */
     ...
     * @param preWeights Float array with weights of spark types for the pre path.
     * @param postWeights Float array with weights of spark types for the post path.
     * @param sparkTypePercentages Float array of percentages of special spark types to spawn.
     */
    public void sendSpawnWave(int amount, int health, int speed, int spawnTime, float[] preWeights, float[] postWeights,
            float[] sparkTypePercentages)
    {
        socket.send("spawnWave " + amount + ";" + Arrays.toString(preWeights) + ";" + Arrays.toString(postWeights) + ";" + health + ";"
                + speed + ";" + spawnTime + ";" + Arrays.toString(sparkTypePercentages));
    }
    ...
    /**
     * Send the boolean value of if the hash compared to the hash in the back end to the client
     ...
     */
    public void startNewQuestion(GameState state, AbstractLogger logger, int challengeCounter)
    {
        ...
        logger.log(problem.getVariableTypes());
