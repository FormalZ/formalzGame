...
public class GameState
{
     ...
     */
    public GameState(AbstractLogger logger)
    {
        this.logger = logger;
        phase = 0;
     ...
     * @return The problem that needs to be solved next. This is either a random problem or the teacher problem.
     */
    public Problem getProblem()
    {
        ...
        if (currentProb == null)
        {
            ...
            {
                isRepo = false;
                logger.log("Reached final problem");
            ...
            else
            {
                ...
                currentProb = ProblemSelector.getRandomProblem(minimumDifficulty, maximumDifficulty, features, oldIds, teacherProblem.getLives(), teacherProblem.getMoney(), teacherProblem.getDeadline());

                adaptiveDifficulty.resetFeatureUsage();
                oldIds.add(currentProb.getId());
                id = currentProb.getId();
                logger.log("Currently doing repo problem with id " + id);
    ...
    /**
     * Start the game.
     * @return A boolean value representing whether the game is successfully started.
     */
    ...
    public void processPreWrongAnswer()
    {
        ...
        adaptiveDifficulty.setMistakeCount(currentLocalStatistic.getPreMistakeCount(), currentLocalStatistic.getPostMistakeCount());
    }
    ...
    private void processGameFinished()
    {
        ...
        int totalTime = localStatistics.stream().map(LocalStatistic::getTimeSpent).mapToInt(i -> i.intValue()).sum() / 1000;
