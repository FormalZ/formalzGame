...
import haskellapi.jsonrunnables.JsonAccumulatorRunnable;
...
public class GameLogicTask extends GameTask {
    ...
    private void check(Supplier<Response> getResponse, BiConsumer<Problem, Response> processResponse,
        ...
        if (!(model != null && model.equals(""))) {
            logger.log("counterexample " + model);
        }
    ...
    private void incorrectResponseCode(int responseCode) {
        ...
        switch (responseCode) {
            case 400:
                ...
                client.getLogger().log("The solution checker has failed");
                break;
            case 500:
                connection.sendWarning("There is probably something wrong with your syntax");
                client.getLogger().log("There is probably something wrong with your syntax");
                break;
            default:
                connection.sendWarning("Something went wrong while checking your solution");
                client.getLogger().log("Something went wrong while checking your solution");
                break;
        }
    }
    ...
    private void processPreResponse(Problem problem, Response response) {
        ...
        logger.log("answer is " + String.valueOf(response.isEquivalent()));
    ...
    private void waveDone(String arguments) {
        ...
        if (problemState.problemCorrect()) {
            ...
            logger.log("Question " + state.getProblemNumber() + " is answered correctly");
            state.processCorrectAnswer();
            logger.log("Time taken " + state.getCurrentStatistic().getTimeSpent());
            if (state.isFinished()) {
                tracker.sendGameEnd();
                logger.log("Game finished");
                connection.sendFinish();
            } else {
                connection.startNewQuestion(state, logger, 0);
